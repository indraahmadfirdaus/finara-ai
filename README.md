...
#          NEXT_PUBLIC_APP_URL (your deployment URL — used in email verification links)
...
Before deploying: in Supabase → Authentication → URL Configuration, set **Site URL** and add `https://yourdomain.com/auth/callback` to **Redirect URLs**.
...
- Dashboard with spending chart, budget progress, balance summary, trend sparklines
- Goals (savings targets with deposits)
- Debts & IOUs (owe/lent tracking, settle flow)
- Budget limits per category
- OCR receipt scanning (camera or image upload)
