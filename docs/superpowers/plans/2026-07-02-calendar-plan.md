...
**Goal:** Add a `/calendar` page under the "Daftar" nav group that shows a month grid with per-day income/expense summaries and a bottom-sheet drawer listing transactions for the selected day.

**Architecture:** Single client component (`calendar/page.tsx`) fetches all transactions for the active month in one bulk request, aggregates them client-side into a `Map<string, DayData>`, renders a 7-column grid, and opens a `DayDrawer` bottom sheet on date tap — no new API endpoints, no extra requests per day.

**Tech Stack:** Next.js 16, React, Framer Motion 12, Tailwind v4, Supabase (existing `/api/transactions` route), lucide-react.

## Global Constraints

- Next.js version: 16.2.9 — auth guard in `src/proxy.ts` (not `middleware.ts`)
- All colors via CSS variables — never hardcode hex values
- Amounts: `formatCompactIDR()` in date cells, `formatIDR()` in drawer
- Dates: always use helpers from `src/lib/utils/date.ts`
- Page wrapped in `<PageTransition>` (`src/components/layout/PageTransition.tsx`)
- No secrets in client-side code
- Run `npx tsc --noEmit` before every commit — zero type errors
- Framer Motion: card enter `stiffness: 300, damping: 22`; list stagger `staggerChildren: 0.04`; nav pill `layoutId="side-pill"` (SideNav) / `"nav-pill"` (BottomNav)
- Category icons: `getCategoryMeta(category, type)` from `src/lib/utils/categoryIcon.tsx`
...
|---|---|---|
| `src/app/(app)/calendar/page.tsx` | Create | Full calendar page — state, fetch, aggregate, render grid + drawer |
| `src/components/layout/SideNav.tsx` | Modify | Add `{ href: '/calendar', icon: CalendarDays, label: 'Kalender' }` to `LIST_SUBS` |
| `src/components/layout/BottomNav.tsx` | Modify | Add same entry to `LIST_SUBS` |

---

## Task 1: Add "Kalender" to nav (SideNav + BottomNav)

**Files:**
- Modify: `src/components/layout/SideNav.tsx` (LIST_SUBS array, ~line 15)
- Modify: `src/components/layout/BottomNav.tsx` (LIST_SUBS array, ~line 9)

**Interfaces:**
- Produces: `/calendar` route is reachable from nav on both desktop sidebar and mobile bottom-nav drawer

- [ ] **Step 1: Add CalendarDays import and entry to SideNav**

Open `src/components/layout/SideNav.tsx`. The current import line is:
```tsx
import {
...
- [ ] **Step 2: Add CalendarDays import and entry to BottomNav**

Open `src/components/layout/BottomNav.tsx`. The current import line is:
```tsx
import { MessageCircle, LayoutDashboard, List, Target, User, HandCoins, X, PieChart, Landmark } from 'lucide-react'
```
Change to:
```tsx
import { MessageCircle, LayoutDashboard, List, Target, User, HandCoins, X, PieChart, Landmark, CalendarDays } from 'lucide-react'
```

Then find `LIST_SUBS` (line ~9):
```tsx
const LIST_SUBS = [
  { href: '/transactions', icon: List, label: 'Transaksi' },
  { href: '/budgets', icon: PieChart, label: 'Anggaran' },
...
git commit -m "feat(nav): add Kalender to Daftar sub-menu"
```

---

## Task 2: Build the calendar page
...
**Interfaces:**
- Consumes:
  - `GET /api/transactions?date_from=YYYY-MM-DD&date_to=YYYY-MM-DD&limit=500` → `{ transactions: Transaction[] }`
  - `formatIDR(n: number): string` from `@/lib/utils/currency`
  - `formatCompactIDR(n: number): string` from `@/lib/utils/currency`
  - `formatDate(date: string | Date): string` from `@/lib/utils/date`
  - `getMonthLabel(monthKey: string): string` from `@/lib/utils/date`
  - `getCategoryMeta(category: string, type?: string): { icon: LucideIcon, bg: string, color: string }` from `@/lib/utils/categoryIcon`
  - `<TopBar title="Kalender" />` from `@/components/layout/TopBar`
  - `<PageTransition>` from `@/components/layout/PageTransition`
- Produces: `/calendar` page — navigable, renders grid and drawer

- [ ] **Step 1: Create the file with types and data helpers**
...
npx tsc --noEmit
```
Expected: no errors. If `getMonthLabel` is missing from `date.ts`, check the import — it is already exported at line 35 of `src/lib/utils/date.ts`.
...
1. Nav shows "Kalender" in the Daftar group (sidebar on desktop, drawer on mobile)
2. Current month grid renders with correct day alignment (first cell of month matches day of week)
3. Today's date cell has accent border
4. Tapping a date with transactions opens the bottom drawer
5. Drawer shows correct date title, summary amounts, and transaction list with icons
6. Tapping backdrop closes the drawer
7. Prev/next month navigation fetches new data and animates grid

- [ ] **Step 4: Commit**

```bash
git add src/app/(app)/calendar/page.tsx
git commit -m "feat(calendar): add calendar page with day drawer"
```
...
- [x] **Nav task (Task 1)** covers adding to both SideNav and BottomNav — spec §Route & Navigation
- [x] **Page task (Task 2)** covers: data fetch, client aggregation, grid render, summary bar, month nav, day cell states (today, selected, empty, has-transactions), DayDrawer with backdrop, stagger animation, responsive sizing — spec §Data Flow, §Components, §Responsive, §Animations
- [x] **Summary format** — both/income-only/expense-only cases handled in `summaryText` JSX
- [x] **No hardcoded hex** — all colors use CSS variables
- [x] **`formatCompactIDR`** in cells, **`formatIDR`** in drawer and summary bar
- [x] **`formatDate` and `getMonthLabel`** used — both exported from `src/lib/utils/date.ts`
- [x] **`PageTransition`** wraps the whole page
- [x] **`AnimatePresence mode="wait"`** on grid for direction-aware slide
- [x] **DayDrawer** uses `stiffness: 380, damping: 34`
- [x] **pb-safe** on drawer via `env(safe-area-inset-bottom)`
- [x] **`lg:max-w-3xl lg:mx-auto lg:px-6`** on page content
- [x] **Drawer max-w-lg centered on desktop** via `lg:max-w-lg lg:mx-auto lg:left-0 lg:right-0`
- [x] No new API endpoints, no DB schema changes
