...
export default function SideNav() {
  ...
  const { collapsed, toggle: toggleCollapsed } = useSideNav()
        ...
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.span
              ...
              className="text-sm font-medium relative overflow-hidden whitespace-nowrap"
              style={{ color: isActive ? 'var(--accent-light)' : 'var(--text-secondary)' }}
            >
              {label}
            </motion.span>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: '1px solid var(--border-light)', minHeight: 72 }}
      >
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.div
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-base font-bold leading-tight" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>finara</p>
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto overflow-x-hidden">
        ...
        {/* Daftar group — hidden when collapsed, show individual icons */}
        {collapsed ? (
            ...
            {LIST_SUBS.map((sub) => {
              ...
              return (
                <Link
                  ...
                  href={sub.href}
                  title={sub.label}
                  className="flex items-center justify-center px-3 py-2.5 rounded-xl relative"
                  style={{ color: isActive ? 'var(--accent-light)' : 'var(--text-secondary)' }}
                >
                  {isActive && (
                    <motion.div
                  ...
                  <sub.icon
                    size={18}
                    strokeWidth={isActive ? 2.5 : 1.8}
                    style={{ color: isActive ? 'var(--accent-light)' : 'var(--text-muted)', position: 'relative' }}
                  />
                </Link>
              )
            })}
      ...
      {/* Bottom actions */}
      <div className="px-2 py-4 space-y-0.5 flex-shrink-0" style={{ borderTop: '1px solid var(--border-light)' }}>
        <button
          onClick={toggle}
          title={collapsed ? (theme === 'dark' ? 'Light mode' : 'Dark mode') : undefined}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors"
          style={{ color: 'var(--text-secondary)', justifyContent: collapsed ? 'center' : undefined }}
          onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--bg-elevated)')}
          onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
        >
          {theme === 'dark'
